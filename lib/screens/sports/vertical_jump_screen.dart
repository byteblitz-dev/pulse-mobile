...
import '../../services/mock_vertical_jump_service.dart';
...
class _VerticalJumpScreenState extends State<VerticalJumpScreen>
  ...
  // Mock service
  final MockVerticalJumpService _mockService = MockVerticalJumpService();
  ...
  double _averageJumpHeight = 0.0;
  double _bestJumpHeight = 0.0;
  ...
  void _setupMockService() {
    _mockService.onCountUpdate = (count) {
      setState(() {
        _jumpCount = count;
        _averageJumpHeight = _mockService.getAverageJumpHeight();
        _bestJumpHeight = _mockService.getBestJumpHeight();
      });
    };
    
    _mockService.onHeightUpdate = (height) {
      setState(() {
  ...
  Future<void> _takeScreenshot() async {
    try {
      ...
      final fileName = 'vertical_jump_test_$timestamp.txt';
      final filePath = '${directory.path}/$fileName';
      
      final testData = '''
╔══════════════════════════════════════════════════════════════╗
║                    VERTICAL JUMP TEST REPORT                ║
╠══════════════════════════════════════════════════════════════╣
║ Test Date: ${DateTime.now().toString()}                    ║
║ Test Mode: Demo (Mock) - AI Tracking Simulation             ║
╠══════════════════════════════════════════════════════════════╣
║ RESULTS:                                                     ║
║ • Total Jumps: $_jumpCount                                  ║
║ • Current Jump Height: ${_currentJumpHeight.toStringAsFixed(1)} cm                    ║
║ • Average Jump Height: ${_averageJumpHeight.toStringAsFixed(1)} cm                  ║
║ • Best Jump Height: ${_bestJumpHeight.toStringAsFixed(1)} cm                      ║
    ...
    try {
      final testData = {
        ...
        'results': {
          ...
          'currentJumpHeight': _currentJumpHeight,
          'averageJumpHeight': _averageJumpHeight,
          'bestJumpHeight': _bestJumpHeight,
                ...
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      child: Text(
                        'Current: ${_currentJumpHeight.toStringAsFixed(1)} cm',
                        style: const TextStyle(
                          color: Colors.white,
                          fontSize: 18,
                          fontWeight: FontWeight.w500,
                    ...
                    if (_jumpCount > 0)
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Text(
                              'Avg: ${_averageJumpHeight.toStringAsFixed(1)} cm',
                              style: const TextStyle(color: Colors.white70, fontSize: 14),
                            ),
                            const SizedBox(width: 16),
                            Text(
                              'Best: ${_bestJumpHeight.toStringAsFixed(1)} cm',
                              style: const TextStyle(color: Colors.white70, fontSize: 14),
                            ),
                          ],
                        ),
